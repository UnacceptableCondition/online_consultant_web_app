...
tests("parse method must returns valid messages data",
    function test(assert) {
        ...
        assert.equal(messageData.sender, "Test121212212121", "it has valid sender field");
    });
tests("parse method must returns valid new message data",
    function test(assert) {
        var messageData = longPollResponseParser.parse(testNewMessage).object["-LH8Qv-7P4sXPrb-uqUq"];
...
tests("parse method must returns valid user list data",
    function test(assert) {
        var user = longPollResponseParser.parse(testUserList).object["Ivan1529913706139"];
...
tests("parse method must returns valid user list data",
    function test(assert) {
        ...
        assert.equal(onlineUpdate.object[0], "Test1212122121211530882419518", "it has valid id field");
        assert.equal(onlineUpdate.object[1], 1531315098909, "it has valid lastOnline field");
    });
